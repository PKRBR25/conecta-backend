...
def test_password_recovery(client, mock_db):
    ...
    with patch("app.core.email.send_reset_password_email") as mock_send_email:
        ...
        response = client.post(f"{settings.API_V1_STR}/auth/password-recovery/test@example.com")
...
def test_register_weak_password(client, mock_db):
    ...
    test_cases = [
        {"password": "weak", "expected_error": "Password must be at least 8 characters long"},
        {"password": "weakpassword", "expected_error": "Password must contain at least one uppercase letter"},
        {"password": "WEAKPASSWORD", "expected_error": "Password must contain at least one lowercase letter"},
        {"password": "WeakPassword", "expected_error": "Password must contain at least one number"},
        {"password": "WeakPassword1", "expected_error": "Password must contain at least one special character"},
    ]
...
def test_reset_password_expired_token(client, mock_db):
    ...
    mock_token = PasswordResetToken(
        user_id=1,
        token="123456",
        expires_at=datetime.utcnow() - timedelta(minutes=1)  # Token expired 1 minute ago
    )
    mock_db.exec.return_value.first.return_value = mock_token
